    ...
    def train(self, training_data_path):
        ...
        for line in lines:
            ...
            for idx, columnValue in enumerate(vector):
                ...
                if self.format[idx] == "num":
                    self.counts[category].setdefault(feature_key, [])
                    self.counts[category][feature_key].append(columnValue)
                elif self.format[idx] == 'attr':
                    self.counts[category].setdefault(feature_key, {})
                    self.counts[category][feature_key].setdefault(columnValue, 0)
                    self.counts[category][feature_key][columnValue] += 1
            ...
            for (col, valueCounts) in columns.items():
                ...
                if self.format[idx] == "num":
                    ...
                    for (attrValue, count) in valueCounts.items():
                        ...
                        #     (count + self.alpha) / (self.classes[category] + self.alpha * len(self.format)))
                        self.conditional[category][col][attrValue] = (
                            (count + self.alpha) / (self.classes[category] + self.alpha * len(valueCounts)))
            ...
            for idx, attrValue in enumerate(itemVector):
                ...
                if self.format[idx] == "num":
                    ...
                    if attrValue not in self.conditional[category][feature_key]:
                        ...
                        prob = prob + (
                            # self.alpha / (
                            #     self.classes[category] + self.alpha * len(self.format)))
                            self.alpha / (
                                self.classes[category] + self.alpha * len(self.counts[category][feature_key])))
                    else:
